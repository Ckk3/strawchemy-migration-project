#!/usr/bin/env python3
"""
Generate GraphQL schema SDL files from the strawchemy project.

This script exports all GraphQL schemas defined in the strawchemy-migrated-project
...
Usage:
    ...
    but can also be run standalone:
        ...
        uv run python ../utils/generate_strawchemy.py <output_dir>

Example:
    uv run python ../utils/generate_strawchemy.py ../schemas/strawchemy
"""
...
SCHEMAS_TO_EXPORT = [
    SchemaConfig(
        ...
        description="Main schema with basic EmployeeType and DepartmentType",
    ),
    SchemaConfig(
        module_name="schema_filters",
        schema_attribute="schema_with_filters",
        output_filename="filters.graphql",
        description="Schema demonstrating filtering capabilities (EXCLUSIVE FEATURE)",
    ),
    SchemaConfig(
        module_name="schema_ordering",
        schema_attribute="schema_with_ordering",
        output_filename="ordering.graphql",
        description="Schema demonstrating ordering/sorting (EXCLUSIVE FEATURE)",
    ),
    SchemaConfig(
        module_name="schema_pagination",
        schema_attribute="schema_with_pagination",
        output_filename="pagination.graphql",
        description="Schema demonstrating offset pagination (EXCLUSIVE FEATURE)",
    ),
    SchemaConfig(
        module_name="schema_pagination",
        schema_attribute="schema_with_child_pagination",
        output_filename="child-pagination.graphql",
        description="Schema demonstrating pagination on nested relationships",
    ),
    SchemaConfig(
        module_name="schema_aggregations",
        schema_attribute="schema_with_relationship_aggregations",
        output_filename="relationship-aggregations.graphql",
        description="Schema demonstrating aggregations on relationships (EXCLUSIVE FEATURE)",
    ),
    SchemaConfig(
        module_name="schema_aggregations",
        schema_attribute="schema_with_root_aggregations",
        output_filename="root-aggregations.graphql",
        description="Schema demonstrating root-level aggregations",
    ),
    SchemaConfig(
        module_name="schema_mutations",
        schema_attribute="schema_with_mutations",
        output_filename="mutations.graphql",
        description="Schema demonstrating CRUD mutations (EXCLUSIVE FEATURE)",
    ),
]
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Generate GraphQL SDL files from strawchemy schemas",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    python generate_strawchemy.py ../schemas/strawchemy
        """,
    )
    parser.add_argument(
        "output_dir",
